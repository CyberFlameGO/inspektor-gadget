...
// AdditionalPodNetworkInformation is for now a specific interface for trace network
// TODO: More granualar and "shareable" interfaces
type AdditionalPodNetworkInformation interface {
	// Local/Host
	SetPodOwner(string)
	SetPodHostIP(string)
	SetPodIP(string)
...
func (k *KubeIPResolver) CanOperateOn(gadget gadgets.GadgetDesc) bool {
	...
	_, hasAdditionalInf := gadget.EventPrototype().(AdditionalPodNetworkInformation)
...
func (m *KubeIPResolverInstance) enrich(ev any) {
	additionalInfo, _ := ev.(AdditionalPodNetworkInformation)
